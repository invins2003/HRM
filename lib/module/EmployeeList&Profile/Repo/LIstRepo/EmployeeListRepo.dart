...
Future<OverTimeModel> updateOverTime({
  ...
  Future<Response> deleteEmployeeRepo( String empId) async {
    debugPrint("ApiClient.deleteData ---> ${Constants.DELETEEMPLOYEEATTENDENCE}/${empId}");
...
Future<LeaveTypeModel> getLeaveTypes() async {
   ...
   Future<Response> createLeave(LeaveModel leave) async {
    ...
    return await apiClient.postData(Constants.CREATEEMPLOYEELEAVE, leave.toJson());
  }

  Future<Response> deleteLeave(String leaveId) async {
  debugPrint("ApiClient.deleteData ---> ${Constants.CREATEEMPLOYEELEAVE}/$leaveId");
  return await apiClient.deleteData("${Constants.CREATEEMPLOYEELEAVE}/$leaveId");
}
...
Future<Response> updateLeave({
 ...
 Future<List<TerminationType>> getTerminationTypes() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to fetch termination types: ${response.statusCode}');
    }
  }
  ...
  if (response.statusCode == 200 || response.statusCode == 201) {
    ...
    throw Exception(response.body['message'] ?? "Failed to create termination");
  }
}
...
Future<ResignationResponse> createResignation({
  ...
  if (response.statusCode == 200 || response.statusCode == 201) {
    ...
    throw Exception(response.body['message'] ?? "Failed to create resignation");
  }
